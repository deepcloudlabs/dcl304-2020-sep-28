export function getFindEmployees(dispatch) {
    let idx= Math.floor(Math.random()*1000);
    return () => {
        fetch('http://localhost:9100/hr/api/v1/employees?_='+idx)
            .then(res => res.json())
            .then(emps => dispatch({type: 'all', employees: emps}));
    };
}

export function refreshEmployees(dispatch,employee) {
     return () => {
         return dispatch({type: 'refresh', employee});
     }
};
